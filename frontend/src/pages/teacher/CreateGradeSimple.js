...
const CreateGradeSimple = () => {
  ...
  // Local state for dropdown options - we'll fetch these directly with axios
          ...
          <Grid container spacing={3}>
                ...
                >
                  {subjects.length > 0 ? (
                    ...
                    <MenuItem disabled value="">
                      {loading ? 'Loading subjects...' : 'No subjects available'}
                    </MenuItem>
                  )}
                </Select>
                {formErrors.subject && (
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                <InputLabel id="direction-filter-label">Filter by Direction (Optional)</InputLabel>
                <Select
                  ...
                  {filteredStudents.length > 0 ? (
                    filteredStudents.map((student) => (
                      <MenuItem key={student._id} value={student._id}>
                        {student.name} 
                        {student.direction && typeof student.direction === 'object' && 
                          ` (${student.direction.name})`}
                      </MenuItem>
                    ))
                  ) : (
                    <MenuItem disabled value="">
                      {loading ? 'Loading students...' : formData.subject ? 'No students found' : 'Select subject first'}
                    </MenuItem>
                  )}
                </Select>
                {formErrors.student && (
