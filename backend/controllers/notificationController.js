...
const getAllNotifications = asyncHandler(async (req, res) => {
  try {
    ...
    if (user.role === 'student') {
      ...
      // Admins can see all notifications in their school
      // No additional filtering needed - they see everything in their school
    }
    else if (user.role === 'superadmin') {
      // Superadmin can see all notifications across all schools
      if (user.schoolId) {
        // If superadmin belongs to a specific school, filter by that school
        query.schoolId = user.schoolId;
      } else {
        // If no specific school, remove schoolId filter to see all
